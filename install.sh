...
# 
# Usage: bash -c "$(wget -qLO - https://your-host/openclaw-lxc.sh)"
#    or: bash openclaw-lxc.sh
# ==============================================================================
...
check_root() {
    [[ $EUID -ne 0 ]] && { msg_error "Must run as root on Proxmox host."; exit 1; }
}

check_proxmox() {
    command -v pveversion &>/dev/null || { msg_error "Must run on Proxmox VE host."; exit 1; }
...
select_storage() {
    ...
    if [[ $count -eq 1 ]]; then
        ...
        local i=1; while IFS= read -r s; do echo "  $i) $s"; ((i++)); done <<< "$storages"
        echo ""; read -rp "Select storage (1-$count) [1]: " choice
...
configure() {
    ...
    local default_id; default_id=$(next_id)
    read -rp "  Container ID [$default_id]: " CT_ID; CT_ID=${CT_ID:-$default_id}
    read -rp "  Hostname [openclaw]: " HN; HN=${HN:-openclaw}
    ...
    if [[ "$var_ollama" != "yes" ]]; then
        read -rp "  External Ollama URL (blank=none): " var_ollama_url_input
        [[ -n "$var_ollama_url_input" ]] && var_ollama_url="$var_ollama_url_input" || var_ollama_url=""
    fi
    ...
    if [[ "$var_webui" == "yes" ]]; then
        ...
        [[ "$var_webui_type" == "caddy" ]] && { read -rp "  Domain (for auto-HTTPS, blank=skip): " var_domain; }
    fi

    read -rp "  Install Management Dashboard? (yes/no) [$var_dashboard]: " i; var_dashboard=${i:-$var_dashboard}
    [[ "$var_dashboard" == "yes" ]] && { read -rp "  Dashboard port [$var_dashboard_port]: " i; var_dashboard_port=${i:-$var_dashboard_port}; }
    ...
    if [[ "$var_ollama" == "yes" ]]; then
        [[ "$var_ram" -lt 4096 ]] && { echo -e "\n  ${YW}⚠ Ollama needs ≥4GB RAM. Bumping to 4096 MB.${CL}"; var_ram=4096; }
        [[ "$var_disk" -lt 16 ]] && { echo -e "  ${YW}⚠ Ollama models need space. Bumping disk to 16 GB.${CL}"; var_disk=16; }
    fi
...
download_template() {
    ...
    local template
    template=$(pveam available --section system 2>/dev/null | grep "debian-${var_version}" | sort -t- -k2 -V | tail -1 | awk '{print $2}')
    [[ -z "$template" ]] && { msg_error "Template not found"; exit 1; }
...
create_container() {
    ...
    [[ "$var_net" == "dhcp" ]] && net+=",ip=dhcp" || { net+=",ip=${var_net}"; [[ -n "$var_gate" ]] && net+=",gw=${var_gate}"; }
    [[ -n "$var_mac" ]] && net+=",hwaddr=${var_mac}"
    [[ -n "$var_vlan" ]] && net+=",tag=${var_vlan}"
    local dns_opt=""; [[ -n "$var_dns" ]] && dns_opt="--nameserver ${var_dns}"
    ...
    pct create "$CT_ID" "$TEMPLATE" \
        ...
        --onboot 1 --ostype "$var_os" --start 0 $dns_opt &>/dev/null
...
start_container() {
    ...
    pct start "$CT_ID" &>/dev/null; sleep 3
    local r=10; while [[ $r -gt 0 ]]; do
...
install_docker() {
    ...
    '
    msg_ok "Docker: $(pct exec "$CT_ID" -- docker --version 2>/dev/null)"
}

# ─── Install Ollama ──────────────────────────────────────────────────────────
install_ollama() {
    [[ "$var_ollama" != "yes" ]] && return
    ...
    '
    local ollama_ver
    ollama_ver=$(pct exec "$CT_ID" -- ollama --version 2>/dev/null || echo "installed")
    msg_ok "Ollama ${ollama_ver}"
...
NGINX
        ...
        nginx -t &>/dev/null
        systemctl enable nginx &>/dev/null && systemctl restart nginx
    "
...
install_dashboard() {
    ...
    pct exec "$CT_ID" -- bash -c "
        ...
        mv /tmp/server.js /opt/openclaw-webui/
        mv /tmp/package.json /opt/openclaw-webui/

        # Environment config
        cat > /opt/openclaw-webui/.env << ENVEOF
WEBUI_PASSWORD=${PASS}
...
ENVEOF
        ...
        # Systemd service
        cat > /etc/systemd/system/openclaw-webui.service << 'SVCEOF'
[Unit]
...
SVCEOF
        ...
        systemctl enable openclaw-webui &>/dev/null
        systemctl start openclaw-webui
    "
    msg_ok "Dashboard installed (port ${var_dashboard_port}, password: ${PASS})"
    DASHBOARD_PASS="$PASS"
}
...
install_openclaw() {
    ...
    pct exec "$CT_ID" -- bash -c "apt-get install -y -qq curl git sudo ca-certificates gnupg openssl" &>/dev/null
    ...
    "
    msg_ok "Node.js $(pct exec "$CT_ID" -- node -v 2>/dev/null)"

    msg_info "Installing pnpm"
    pct exec "$CT_ID" -- bash -c "npm install -g pnpm@latest &>/dev/null"
    ...
    pct exec "$CT_ID" -- bash -c "
        ...
        $([ "$var_docker" == "yes" ] && echo 'usermod -aG docker openclaw 2>/dev/null || true')
    "
    ...
    local sandbox=""
    [[ "$var_docker" == "yes" ]] && sandbox=', "sandbox": {"enabled": true, "provider": "docker"}'
    
    local controlui=""
    [[ "$var_webui" == "yes" ]] && controlui=', "controlUi": {"enabled": true, "allowedOrigins": ["*"]}'

    pct exec "$CT_ID" -- bash -c "
        su - openclaw -c 'cat > ~/.openclaw/openclaw.json << OCEOF
{
  \"gateway\": {
    \"port\": 18789,
    \"mode\": \"local\",
    \"bind\": \"loopback\"${controlui}
  },
  \"env\": {
    ${provider_env}
  },
...
OCEOF'
    "
    msg_ok "OpenClaw configured (provider: $([ -n "$var_ollama_url" ] && echo "Ollama" || echo "Anthropic"))"
...
"
    ...
    [[ "$var_ssh" == "yes" ]] && {
        ...
        pct exec "$CT_ID" -- bash -c "apt-get install -y -qq openssh-server &>/dev/null; systemctl enable ssh &>/dev/null; systemctl start ssh"
        msg_ok "SSH enabled"
    }
}
...
show_completion() {
    ...
    echo ""
    echo -e "${GN}╔══════════════════════════════════════════════════════════╗${CL}"
    echo -e "${GN}║         ✓ ${APP} LXC Installation Complete!              ║${CL}"
    echo -e "${GN}╚══════════════════════════════════════════════════════════╝${CL}"
    echo ""
    echo -e "  ${BL}Container:${CL}   ${CT_ID} / ${HN}"
    [[ -n "$ip" ]] && echo -e "  ${BL}IP Address:${CL}  ${ip}"
    echo ""

    echo -e "  ${BL}── Installed Services ──${CL}"
    echo -e "  ✓ OpenClaw AI Gateway          (port 18789)"
    [[ "$var_docker" == "yes" ]]    && echo -e "  ✓ Docker CE                     (sandbox provider)"
    ...
    if [[ "$var_webui" == "yes" ]]; then
        if [[ "$var_webui_type" == "caddy" && -n "${var_domain:-}" ]]; then
            echo -e "  Control UI:   ${GN}https://${var_domain}${CL}"
        else
            echo -e "  Control UI:   ${GN}http://${ip:-<ip>}:${var_webui_port}${CL}"
        fi
    fi
    [[ "$var_dashboard" == "yes" ]] && echo -e "  Dashboard:    ${GN}http://${ip:-<ip>}:${var_dashboard_port}${CL}"
    [[ "$var_ollama" == "yes" ]]    && echo -e "  Ollama API:   ${GN}http://${ip:-<ip>}:11434${CL}"

    if [[ "$var_dashboard" == "yes" && -n "${DASHBOARD_PASS:-}" ]]; then
        echo ""
        echo -e "  ${YW}🔑 Dashboard Password: ${GN}${DASHBOARD_PASS}${CL}"
    ...
    echo ""
    echo -e "  ${GN}1.${CL} Enter the container:        ${GN}pct enter ${CT_ID}${CL}"
    echo -e "  ${GN}2.${CL} Switch to openclaw user:    ${GN}su - openclaw${CL}"
    echo -e "  ${GN}3.${CL} Run setup wizard:           ${GN}openclaw setup${CL}"

    if [[ "$var_ollama" == "yes" ]]; then
        echo -e "  ${GN}4.${CL} Pull an Ollama model:      ${GN}ollama pull llama3${CL}"
        echo -e "     ${DIM}Or use the Dashboard to pull models from the browser!${CL}"
    else
        echo -e "  ${GN}4.${CL} Set your API key:          ${GN}openclaw config${CL}"
    fi

    echo -e "  ${GN}5.${CL} Connect a channel:          ${GN}openclaw channels login${CL}"
    echo -e "  ${GN}6.${CL} Start the gateway:          ${GN}sudo systemctl start openclaw${CL}"
    echo ""
    echo -e "  ${BL}Docs:${CL}       https://docs.openclaw.ai"
    echo -e "  ${BL}Community:${CL}  https://discord.com/invite/clawd"
    echo ""
    echo -e "${GN}══════════════════════════════════════════════════════════${CL}"
}
